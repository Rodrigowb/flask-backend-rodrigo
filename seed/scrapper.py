...
class ZillowScraper():
  ...
  URL = 'https://www.zillow.com/manhattan-new-york-ny/fsbo/'

  def __init__(self):
    """
    Define empty list to store the parsed json object
    Define headers to avoid captcha pop ups
    Define the page ranages that we want to scrape
    """
    self.results = []
    self.headers = {
  ...
  def __fetch(self, url, params):
    """
    Fetch a GET request to a specific url and given params
    """
  ...
  def __parse(self, response):
    """
    Parse to lxml the fetched response
    Append the values that we want to the array
    """
  ...
  def run(self):
    ...
    for page in self.range:
      ...
      print(f'{(page/len(self.range))*100}%...')
